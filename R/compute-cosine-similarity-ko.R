...
# KO names
id_list <- colnames( matrix_df )

# Get number of KOs
n_kos <- length( id_list )

# Get a list of row vectors
vectors <- lapply( 1:n_kos, function( n ) as.vector( matrix_df[,n] ) )

# Estimate the number of dot products we will compute
n_dots <- n_kos * (n_kos + 1)/2

# Define pattern for intermediate files
filepattern <- "temporary-data/ko-dot-products/dots-%d.rds"
...
dot_products <- bind_rows( lapply( 1:n_kos, function( a ) readRDS( sprintf( filepattern, a ) ) ) )
...
saveRDS( similarities, "processed-data/ko-cosine-similarities.rds" )
