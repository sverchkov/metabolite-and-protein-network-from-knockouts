...
protein_map <- bind_rows(
  mapply(
    function ( molecule_id, uniprot_id ){
      tibble( id = molecule_id, uniprot = paste0( "up:", uniprot_id ) )
    },
    proteins %>% pull( `Molecule ID` ) , strsplit( proteins %>% pull( `Protein ID` ), "; *" ),
    SIMPLIFY = F )
) %>%
...
protein_pathway <- distinct( protein_map, a = id, a_hsa = hsa_code )
